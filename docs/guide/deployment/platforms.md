...
Choose a runtime based on what your app needs. Pondoknusa **1.x** targets **Node.js 26+** (or Bun) as the primary production runtime. Edge runtimes are supported for **partial** workloads today; a first-class edge adapter is on the roadmap.
...
| Future: git-push managed hosting | [Pondoknusa Cloud](/guide/deployment/pondoknusa-cloud) (planned) |

## Feature matrix

| Capability | Docker / VPS | Fly / Railway | Cloudflare (edge) | Pondoknusa Cloud (planned) |
|------------|----------------|---------------|-------------------|-------------------------|
| Full Pondoknusa (SSR + API + queues) | Yes | Yes | Partial — origin required | Yes |
| `pondoknusa start --cluster` | Yes | Yes | No | Yes |
...
```
Browser → CDN (optional) → Node (pondoknusa start) → Postgres
                              ↳ queue worker
                              ↳ scheduler cron
```
...
Browser → Cloudflare Pages (static SPA)
       → Cloudflare proxy → Node Pondoknusa API (Fly/Railway)
       → R2 (uploads)
```

Common when the UI is Vite/React and the API is `pondoknusa new --headless`.
...
pondoknusa deploy:check  # optional gate before traffic
```
